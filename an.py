...
@app.route('/visualize')
def visualize():
    path = 'C:/Users/최윤수/Desktop/monthly_an'
    ...
    for file_name in file_list:
        ...
        data = {'Date': [file_name.replace(path + '/', '').split('.')[0]],
                ...
                '평균단가': d}
        i = pd.DataFrame(data)
        an_all = pd.concat((an_all, i), axis=0, ignore_index=True)
    print(an_all)
    ...
    sns.lineplot(data=an_all, x='Date', y='최고단가', label='최고단가', marker='s')
    sns.lineplot(data=an_all, x='Date', y='평균단가', label='평균단가', marker='s')
    sns.lineplot(data=an_all, x='Date', y='최저단가', label='최저단가', marker='s')
    #sns.set(font=font_name)

    plt.ylabel("PRICE")
    plt.xlabel("DATE")
    plt.title('Monthly_Angae', fontsize=10)
